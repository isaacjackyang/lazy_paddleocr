param(
    ...
    [string]$DockerfilePath = ".\distribution_tools\Dockerfile",
...
try {
    ...
    if ($AutoStart) {
        ...
        $toolRoot = if ($PSScriptRoot -and (Test-Path $PSScriptRoot)) { $PSScriptRoot } else { $projectRoot }
        $startScript = Join-Path $toolRoot "start_ocr_launcher_docker.ps1"
