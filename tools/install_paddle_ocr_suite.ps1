...
function Get-ManualRecoveryItems {
    ...
    if (-not $script:InstallStageStatus.paddle) {
        if ($Mode -eq "gpu") {
            Add-ManualRecoveryItem -Items $items -Title "Install PaddlePaddle GPU" -Command (New-PipManualCommand -PythonExe $venvPython -Packages @("paddlepaddle-gpu==3.3.1") -ForceReinstall:$false -IndexUrl "https://www.paddlepaddle.org.cn/packages/stable/$Cuda/")
...
function Write-ManualRecoveryChecklist {
    ...
    $projectRoot = Get-ManualProjectRoot
    $outputPath = Join-Path $projectRoot "install_manual_recovery.ps1"
    $fileLines = @(
...
function Invoke-InstallerFromShortDrive {
    ...
    $mappedScript = Join-Path "$substDrive\" (Split-Path $script:InstallerScriptPath -Leaf)
    $argumentList = @(
        "-NoProfile",
...
function Install-PaddleGPU {
    ...
    Invoke-PipInstall `
        -PythonExe $PythonExe `
        -Packages @("paddlepaddle-gpu==3.3.1") `
...
function Start-Launcher {
    ...
    $launcher = Join-Path $ProjectRoot "start_ocr_launcher.ps1"
    if (-not (Test-Path $launcher)) {
        Write-Host "start_ocr_launcher.ps1 not found. Skipping auto-start." -ForegroundColor Yellow
        return
    }
