...
function Wait-ForDockerReady {
    ...
    if ($result.ExitCode -ne 0) {
        throw "Docker engine is not ready. Start Docker Desktop first, or run install_paddle_ocr_docker.ps1."
    }
...
try {
    ...
    if (-not $dockerCli) {
        throw "docker.exe was not found. Run install_paddle_ocr_docker.ps1 first."
    }
    ...
    if (-not (Test-DockerImageExists -DockerCli $dockerCli -Tag $ImageTag)) {
        throw "Docker image not found: $ImageTag. Run install_paddle_ocr_docker.ps1 first."
    }
